...
func (cli *DockerCli) CmdBuild(args ...string) error {
	...
	if cmd.Arg(0) == "-" {
		...
		//)若该参数为 "-' ，则代表 Docker 用户指定 STDIN 作为 Dockerfile 的输入源， Docker Client 随即通过 docker bui1 命令的标准输入读人数据，压缩后最终得到结果 context;
		...
		//若该参数与 URL 匹配，并且不为 git 地址，又或者参数与 URL 匹配，同时也是
		//git 地址，但是 Docker Client 所在宿主机没有安装 git ，则标记 isRemote 参数为 true ，表明
		//Dockerfile 需要远程获取
		isRemote = true
	} else {
		//)若该参数为 git 地址，且本地安装 git ，则 Docker Client 首先将 git 内容通过 git c10ne
		//命令复制到本地;否则，表明 Dockerfile 已经位于当前目录， Docker Client 解析当前路径之后，
		//将内容压缩打包，得到最终结果 context
		root := cmd.Arg(0)
		if utils.IsGIT(root) {
			remoteURL := cmd.Arg(0)
...
func (cli *DockerCli) CmdPull(args ...string) error {
	...
	tag := cmd.String([]string{"#t", "#-tag"}, "", "Download tagged image in a repository") //解析参数
	if err := cmd.Parse(args); err != nil {
		return nil
	}
	...
	//通过 remote 变量首先得到镜像的 reposit。可名称，并赋给 remote 自身，随后通过解析
	//改变后的 remote ，得出镜像所在的 host 地址，即 Docker Registry 的地址。若用户没有制定
	//Docker Registry 的地址，则 Docker 默认地址为 DockerHub 地址 https://index.docker.i o/vl/
	...
	//通过cli对象获取与DockerServer通信所需要的认证配置信息
	cli.LoadConfigFile()
