...
func createRouter(eng *engine.Engine, logging, enableCors bool, dockerVersion string) (*mux.Router, error) {
	...
	r := mux.NewRouter()
	if os.Getenv("DEBUG") != "" {
		AttachProfiler(r) //添加与 DEBUG 相关的路由记录
	}
	//添加handler
	...
	for method, routes := range m {
		for route, fct := range routes {
			...
			if localRoute == "" {
				...
				r.Path("/v{version:[0-9.]+}" + localRoute).Methods(localMethod).HandlerFunc(f) //版本兼容
...
//使 Docker Server 监昕某一指定地址，并接收该地址上的请求，并对以上请求路由转发至相应的处理方法处
//1 )创建 router 路由实例。
//2) 创建 1istener 监听实例。
//3 )创建 http.Se er
//4) 启动 API 服务。
...
/*通过循环多种指定协议，创建出 goroutine 协调来配置
指定的 http.Server ，最终为不同协议的请求服务*/
func ServeApi(job *engine.Job) engine.Status {
	if len(job.Args) == 0 { //)检验 Job 的参数，确保传人参数无误。
		return job.Errorf("usage: %s PROTO://ADDR [PROTO://ADDR ...]", job.Name)
	}
	var ( //定义 Docker Server 的监昕协议与地址，以及错误信息管道 channel
		protoAddrs = job.Args
		chErrors   = make(chan error, len(protoAddrs))
	)

	//se eapi 运行时， ServeFd ListenAndServe 函数
	//均由于 activationLock 中没有内容而阻塞，而当运行 acceptionconnections 这个 Job 时，该 Job
	//会首先通知 init 进程 Docker Daemon 已经启动完毕，并关闭 activationLock ，因此 ServeFd
	//ListenAndServe 不再阻塞，结果是 serveapi 继续执行。正是由于 activationLock 的存在，
	//Docker Daemon 可以保证 acceptconnections 这个 Job 的运行有能力通知 serveapi 开启正式服
	//务于 API 请求的功能。
	activationLock = make(chan struct{})

	for _, protoAddr := range protoAddrs { //遍历协议地址，针对协议创建相应的服务端。
		protoAddrParts := strings.SplitN(protoAddr, "://", 2)
		if len(protoAddrParts) != 2 {
			return job.Errorf("usage: %s PROTO://ADDR [PROTO://ADDR ...]", job.Name)
		}
		go func() { //通过 chErrors 建立 goroutine 与主进程之间的协调关系。
...
//通知宿主机上 init 守护进程 Docker Daemon 已经启动完毕，可以让 Docker Daemon 开始服务API 请求。
