...
var (
	...
	drivers map[string]InitFunc //初始化时会默认注册进去
...
func New(root string, options []string) (driver Driver, err error) {
	...
	for _, name := range priority { //在没有指定以及默认的驱动时，从优先级数组中选择驱动，目前优先级最高的为 aufs
	...
	for _, initFunc := range drivers { //从已经注册的 drivers 数组中选择 graphdriver
