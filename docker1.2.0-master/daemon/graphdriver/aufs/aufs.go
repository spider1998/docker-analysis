...
// mnt, layers, and diff
//layers diff 以及 mnt 为目录 /var/lib/docker/aufs 下的三个子目录，
//1 、 是镜像 ID ，分别代表三个镜像，三个目录下的 均代表同一个镜像 ID 。其中 layers
//目录下保留每一个镜像的元数据，这些元数据是这个镜像的祖先镜像 ID 列表; diff 目录下存
//储每一个镜像所在的 layer ，具体包含的文件系统内容; mnt 目录下每一个文件都是一个镜像
//ID ，代表在该层镜像之上挂载的可读写 layer 。因此，下载的镜像中与文件系统相关的具体内
//容，都会存储在 diff 目录下的某个镜像 ID 目录下。
