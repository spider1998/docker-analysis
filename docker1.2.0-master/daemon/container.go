...
//Start 函数实现了进程的启动，另外在
//启动进程的同时为进程设定了命名空间( namespaces) 并为进程做了资源的限制，从而保证
//进程以及之后进程的子进程都会在相同的命名空间内，且受到相同的资源控制。如此一来，
//Start 函数创建的进程，以及该进程之后的子进程，形成一个进程组，该进程组处于资源隔离
//和资源控制的环境中，我们习惯将这样的进程组环境称为容器，也就是这里的 Docker 容器。
func (container *Container) Start() (err error) {
	...
	//，填充 Docker 容器内部需要执行的命令， Command 中含有进程启 动命令，还含有容器环境的配置信息，也包括网络配置。
	...
	//实现 Docker 容器内部进程的启动，进程启动之后，为容器创建网络环境等。
...
func (container *Container) initializeNetworking() error {
	var err error
	if container.hostConfig.NetworkMode.IsHost() { //host模式
	...
	} else if container.hostConfig.NetworkMode.IsContainer() { //other container 模式
		// we need to get the hosts files from the container to join
		nc, err := container.getNetworkedContainer() //获取制定容器
		if err != nil {
			return err
		}
		//将之前容器对象的 HostsPath ResolveConfpath Hostname Domainname 赋值给当前容器对象
		container.HostsPath = nc.HostsPath
		container.ResolvConfPath = nc.ResolvConfPath
		container.Config.Hostname = nc.Config.Hostname
		container.Config.Domainname = nc.Config.Domainname
	} else if container.daemon.config.DisableNetwork { //none模式Docker 容器的 none 网络模式意味着不给该容器创建任何网络环 境，容器只能使用 127.0. 1.1的环回接口。
		container.Config.NetworkDisabled = true
		return container.buildHostnameAndHostsFiles("127.0.1.1")
	} else {
		//为容器分配一个 网络接口可用的 IP 地址，并将网络配置(包括 IP bridge Gateway 等)赋值给 container 象的 NetworkSettings
		if err := container.allocateNetwork(); err != nil {
			return err
		}
		return container.buildHostnameAndHostsFiles(container.NetworkSettings.IPAddress) //通过 etworkSettings 为容器创建 hosts hostname 等文件。
	}
	return nil
}
