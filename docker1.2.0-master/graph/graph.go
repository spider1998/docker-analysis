...
type Graph struct {
	Root    string                 //graph 的工作根目录，一般为 "/var/lib/docker/graph
	idIndex *truncindex.TruncIndex //idlndex 使得检索字 符串标识符时，允许使用任意一个该字符串唯一的前缀，只要该前缀全局唯一，则可确保找 到相应的镜像。
	driver  graphdriver.Driver     //表示具体的 graphdriver 类型。
}
